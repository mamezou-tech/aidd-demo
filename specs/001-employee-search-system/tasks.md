...
**総タスク数**: 15  
**総ストーリーポイント**: 107 SP  
**推定期間**: 5-7スプリント（2.5-3.5ヶ月）

**実装順序**:
1. タスク1-4: バックエンド基盤（データベース、ドメイン、Repository、認証）
2. タスク5-7: バックエンドAPI（UseCase、Controller）
3. タスク8-10: フロントエンド基盤（型定義、サービス、共通コンポーネント、ログイン）
4. タスク11-13: フロントエンドUI（検索一覧、詳細、ルーティング）
5. タスク14-15: テスト（E2E、パフォーマンス、セキュリティ）

**テストファースト徹底**:
- すべての実装タスクでテスト先行
- Repository層はTestcontainers（実DB）必須
- mockの使用禁止（Repository層）

**プロジェクト憲法遵守**:
- Controllerは入出力変換のみ
- 薄いService層なし
- ドメインロジックはApplication/Domain層
- 3層アーキテクチャ厳守

**完了基準**:
- [ ] すべてのテストが通る（ユニット、統合、E2E）
- [ ] パフォーマンス基準を満たす（SC-002, SC-003, SC-004）
- [ ] すべてのUser Storyの受け入れ基準を満たす
- [ ] ドキュメント完備

**次のステップ**: `/speckit.implement` で個別タスクの実装開始
